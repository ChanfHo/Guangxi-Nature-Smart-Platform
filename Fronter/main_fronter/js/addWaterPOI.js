...
export function addWaterPOI(map, geojsonPath) {
  ...
  function addLayerToMap() {
    ...
    if (map.getLayer('waterPOI-layer')) {
      map.removeLayer('waterPOI-layer');
    }
    if (map.getSource('waterPOI')) {
      map.removeSource('waterPOI');
    }

    // 2. 添加数据源
    map.addSource('waterPOI', {
      type: 'geojson',
      data: geojsonPath,
      generateId: true
    });
    ...
    // 3. 添加自定义图标图层
    map.loadImage('./icons/waterOnMap.png', (error, image) => {
      if (error) {
        console.error('无法加载水域图标:', error);
        return;
      }
      ...
      map.addLayer({
        ...
        source: 'waterPOI',
        layout: {
          'icon-image': 'water-icon', // 使用自定义图标
          'icon-size': 0.1, // 图标默认大小
      ...
      // 添加悬停效果
      addPoiHoverEffect(map, 'waterPOI-layer', {
        nameProperty: '景区名',
        defaultSize: 0.1,
        hoverSize: 0.2
      });
    ...
    // 添加点击事件
    map.off('click', 'waterPOI-layer'); // 移除之前的点击事件
    map.on('click', 'waterPOI-layer', (e) => {
      const coordinates = e.features[0].geometry.coordinates.slice();
      const { 景区名, 所在地, 等级, 介绍, 图片路径 } = e.features[0].properties;

      // 如果图片路径为空，则显示默认图片
      const imagePath = 图片路径 || './geojson/images/water/default.png';
      ...
      new mapboxgl.Popup()
        ...
        .setHTML(`
          <div style="text-align: center;">
            ...
            <img src="${imagePath}" alt="${景区名}" style="width: 220px; height: 180px; margin: 10px 0;" />
            <div>
              所在地: ${所在地}<br>
              等级: ${等级 || '暂无等级'}<br>
              介绍: ${介绍}
